...
g_ee_front_foot = (29*cos(a2 + a3))/8 - (29*cos(a1 + a2 + a3 - a4 - a5))/8 - (7*sin(a1 + a2 + a3 - a4 - a5 - a6))/4 - (29*cos(a1 + a2 + a3 - a4))/8 + (29*cos(a3))/8; %y pos of front foot
...
front_foot_y_str = char(g_ee_front_foot);
...
% Previous constraint not used anymore
...
Constr = constraint(x0,g_ee_x_str,x0,'initial', xVars) + ... % x(0)
    ...
    constraint(0,front_foot_y_str,com_height_top,'trajectory',xVars) + ... %
...
a_time_anim = a_time() .* 2;
% a_time_anim = a_time(1);
% traj_alpha_anim = traj_alpha(:,1);
my_biped.animateTrajectory(a_time_anim, traj_alpha) 