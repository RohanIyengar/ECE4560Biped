...
g_lf_rfFront = [  cos(a1 + a2 + a3 - a4 - a5 - a6), sin(a1 + a2 + a3 - a4 - a5 - a6), (7*cos(a1 + a2 + a3 - a4 - a5 - a6))/4 - (29*sin(a1 + a2 + a3 - a4 - a5))/8 + (29*sin(a2 + a3))/8 - (29*sin(a1 + a2 + a3 - a4))/8 + (29*sin(a3))/8;
                -sin(a1 + a2 + a3 - a4 - a5 - a6), cos(a1 + a2 + a3 - a4 - a5 - a6), (29*cos(a2 + a3))/8 - (29*cos(a1 + a2 + a3 - a4 - a5))/8 - (7*sin(a1 + a2 + a3 - a4 - a5 - a6))/4 - (29*cos(a1 + a2 + a3 - a4))/8 + (29*cos(a3))/8;
                0, 0, 1];
g_lf_rfFront_x_str = char(vpa(g_lf_rfFront(1,3), 9));
g_lf_rfFront_y_str = char(vpa(g_lf_rfFront(2,3), 9));
...
Constr = constraint(x0,g_ee_x_str,x0,'initial', xVars) + ... % x(0)
    ...
    constraint(y0,g_ee_y_str,y0,'trajectory', xVars) + ...
    ...
    constraint(0.7854,g_ee_a1_str,0.7854,'initial', xVars) + ...
    constraint(-1.5708,g_ee_a2_str,-1.5708,'initial', xVars) + ...
    constraint(0.7854,g_ee_a3_str,0.7854,'initial', xVars) + ...
    constraint(1.1437,g_ee_a4_str,1.1437,'initial', xVars) + ...
    constraint(-1.1437,g_ee_a5_str,-1.1437,'initial', xVars) + ...
    constraint(0,g_ee_a6_str,0,'initial', xVars) + ...
    constraint(-.4141,g_ee_a1_str,-.4141,'final', xVars) + ...
    constraint(-0.3533,g_ee_a2_str,-0.3533,'final', xVars) + ...
    constraint(0.7674,g_ee_a3_str,0.7674,'final', xVars) + ...
    constraint(0.6177,g_ee_a4_str,0.6177,'final', xVars) + ...
    constraint(-1.2271,g_ee_a5_str,-1.2271,'final', xVars) + ...
    constraint(0.6094,g_ee_a6_str,0.6094,'final', xVars) + ...
    constraint(-pi/4,g_ee_a3_str,pi/2,'trajectory',xVars) + ... %foot angle constraints
    constraint(0,g_ee_a6_str,pi/2,'trajectory',xVars) + ...
    constraint(0,g_lf_rfFront_y_str,0,'trajectory',xVars) + ... %front of right foot
    constraint(5.75,g_lf_rfFront_x_str,5.75,'initial',xVars) + ...
    constraint(5.75,g_lf_rfFront_x_str,5.75,'trajectory',xVars) + ...
    constraint(-pi/3,g_ee_a1_str,(3*pi)/4,'trajectory',xVars) + ... %Hip angle constraints
    constraint(-pi/3,g_ee_a4_str,(3*pi)/4,'trajectory',xVars) + ...
    constraint(0,pos_com_x_str,com_front+xf,'trajectory',xVars); % COM x constraint
